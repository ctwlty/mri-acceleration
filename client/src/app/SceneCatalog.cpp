...
QList<SceneTemplate> SceneCatalog::defaults()
{
    return {
        makeTemplate(
            ...
            QStringLiteral("标准水模"),
            QStringLiteral("水模横断位 FSE A Mock 基线"),
            QStringLiteral("横断位水模 Mock 结果包：来源记录、PNG 结果与图像级 QC"),
            QStringLiteral("LOC Mock / FSE A Mock"),
            QStringLiteral("横断位 LOC 规划 + 单次 FSE A Mock 执行"),
            QStringLiteral("确认 Mock 水模预设、结果根目录和无设备副作用"),
            QStringLiteral("默认横断位，复核中心、FOV 与切片覆盖"),
            QStringLiteral("使用项目内合法 Mock 图像资产完成标准 Mock 结果与图像级 QC"),
            QStringLiteral("水模 Mock 图像质控；保存协议、参数快照、来源和结果"),
            loc017tParams()
                + QStringLiteral("\nFSE A Mock：参数在开始时冻结为唯一快照；"
                                 "不写入 SDK 或真实参数文件。"),
            QStringLiteral("Mock 结果根可写；Live 的水模/线圈、连接、温度、IDLE 与输出隔离均 BLOCKED"),
            QStringLiteral("Mock 输入来源、PNG 结果、参数快照、QC 与审计事件"),
            QStringLiteral("待 Mock 运行"),
            QStringLiteral("待 Mock 运行"),
            QStringLiteral("待 Mock 运行"),
            QStringLiteral("单次 Mock 执行；不自动重复"),
            QStringLiteral("当前水模横断位 Mock 基线；真实 Run 保持 BLOCKED。")),
        makeTemplate(
            QStringLiteral("结构与形态成像"),
