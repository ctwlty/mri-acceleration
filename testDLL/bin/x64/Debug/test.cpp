...
const char* DLL_PTH = "D:\\123pan\\Downloads\\testDLL\\bin\\x64\\Debug\\mridll.dll";
...
const char* INITFILE = "D:\\123pan\\Downloads\\testDLL\\bin\\x64\\Debug\\hw_cfg\\init.ini";
...
typedef double (*GetParameterPtr)(char *);
...
class dllFunc {
    ...
    GetParameterPtr GetParameter = nullptr;
...
const char *FUNC_NAME[] = {"Init", "ConfigFile", "SetAverageMode", 
        ...
        "GetTotalScanNo", "GetCurrentScanNo", "GetParameter"};
...
int tieFunc(HMODULE dll, dllFunc& df) {
    ...
    df.SetParameterFile = reinterpret_cast<SetParameterFilePtr>(addrs[4]);
    ...
    df.SetParameter = reinterpret_cast<SetParameterPtr>(addrs[7]);
    ...
    df.Abort = reinterpret_cast<AbortPtr>(addrs[12]);
    df.ScanCompleted = reinterpret_cast<ScanCompletedPtr>(addrs[13]);
    ...
    df.GetParameter = reinterpret_cast<GetParameterPtr>(addrs[18]);
    return 0;
}
...
int main(){
    SetCurrentDirectoryA("D:\\123pan\\Downloads\\testDLL\\bin\\x64\\Debug");
    cout << "starting" << endl;
    dllFunc df;
    getFuncFromDll(df);
    if (df.Init == nullptr) {
    ...
    double viewBlock = df.GetParameter("viewBlock");
    double TR = df.GetParameter("TR");
    cout << "viewBlock is " << viewBlock << " and TR is " << TR << endl;
